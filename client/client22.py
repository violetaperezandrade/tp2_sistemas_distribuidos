...
class Client:
    ...
    def run_sender(self):
        signal.signal(signal.SIGTERM, self.handle_sigterm_sender)
    ...
    def __start_connection(self):
        ...
        # Connection arrived
        logging.info(
            f"action: start_connection | host: {self.__address[0]}"
            f"| port: {self.__address[1]} | result: in_progress")
        self._client_socket.connect(self.__address)
        logging.info(
            f'action: start_connection | host: {self.__address[0]} | '
            f'port: {self.__address[1]} | result: success'
        )
    ...
    def __retrieve_server_ack(self):
        ...
        if ack != 8:
            logging.info(
                f'action: receive_message | host: {self.__address[0]} | '
                f'port: {self.__address[1]} | result: error'
    ...
    def handle_sigterm_sender(self, signum, frame):
        logging.info(
